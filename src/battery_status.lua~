...
-- Simply copy the above function of get_acpi_res()
local function get_adapter_res()
  local acpi = io.popen('acpi -a 2>&1') -- 2>&1 to make error go to stdout.
  local acpi_res = acpi:read("*line")
  acpi:close()
  return acpi_res
end
...
local function get_charging_battery_file(percent)
  local img_name = (percent > 90 and "100") or
                   (percent > 70 and "80") or
                   (percent > 50 and "60") or
                   (percent > 30 and "40") or
  ...
  return string.format("%s%s%s.png", icons_dir, img_name,"charging")
end
...
local function check_battery()
  ...
  -- fetching the Adapter 0 status on-line or off-line? 
  local adapter_res = get_adapter_res()
  -- local charger_status = charge_rex:match(adapter_res)
